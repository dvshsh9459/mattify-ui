...
export default function App(){
  ...
  const [productsState, setProductsState] = useState(localProducts)
  const [bannersState, setBannersState] = useState(localBanners)
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        if (res.ok) setProductsState(await res.json())
      } catch (e) {
  ...
  useEffect(() => {
    const fetchBanners = async () => {
      try {
        ...
        if (res.ok) setBannersState(await res.json())
      } catch (e) {
  ...
  // When a category is selected (not 'All'), scroll the filters into view
  useEffect(() => {
    if (selectedCategory !== 'All') {
      const aside = document.querySelector('aside')
      if (aside) {
        // give the layout a moment to render then scroll
        setTimeout(() => aside.scrollIntoView({ behavior: 'smooth', block: 'start' }), 120)
      }
    }
  }, [selectedCategory])
    ...
    <div className="min-vh-100 bg-light d-flex flex-column">
      ...
      <main className="container-fluid px-4 py-4 flex-grow-1">
        ...
        <div className="row g-4">
          ...
          <div className={selectedCategory !== 'All' ? 'col-lg-9' : ''}>
            <div className="d-flex align-items-center justify-content-between mb-3">
              <h3 className="h5 fw-bold">{selectedCategory === 'All' ? 'All Products' : selectedCategory} <span className="text-muted" style={{fontSize: '0.875rem'}}>({filteredProducts.length} items)</span></h3>
